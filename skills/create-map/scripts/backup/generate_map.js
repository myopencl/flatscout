        ...
        function filterMap() {
            ...
            filtered.forEach(apt => {
                try {
                    ...
                    const isFav = (apt.status || '').toUpperCase() === 'SHORTLIST';
                    const favIndicator = isFav ? "<div style='margin-top:6px;'>⭐ Favorito</div>" : "";
                    const addFavCmd = '/detalle ' + (apt.id || '') + ' marcar_shortlist';
                    const removeFavCmd = '/detalle ' + (apt.id || '') + ' quitar_shortlist';
                    const addFavUrl = 'https://t.me/myflatscout_bot?text=' + encodeURIComponent(addFavCmd);
                    const removeFavUrl = 'https://t.me/myflatscout_bot?text=' + encodeURIComponent(removeFavCmd);

                    const favButtons = isFav
                        ? ("<a class='btn-bot' href='"+removeFavUrl+"' target='_blank'>✖ Quitar favorito</a>")
                        : ("<a class='btn-bot' href='"+addFavUrl+"' target='_blank'>⭐ Añadir a favoritos</a>");

                    const m = L.circleMarker(apt.coords, { radius: 12, fillColor: color, color: '#fff', weight: 2, fillOpacity: 1 }).addTo(map)
                        .bindPopup("<div class='popup-content'><b>"+apt.address+"</b><br><div class='price'>"+Number(apt.price || 0).toLocaleString()+" PLN</div>📐 "+apt.size+" m² | 🛏️ Rooms: "+apt.rooms+" | 🏢 Floor: "+apt.floor+"<br>💰 Gastos: "+fmtMonthly(apt.monthly)+"<br>⭐ Score: "+Number(apt.score || 0)+"/100<br>🏷️ Estado: "+(apt.status || 'N/D')+visitLine+commentsLine+favIndicator+"<br><a href='"+apt.url+"' target='_blank'>Ver anuncio</a>"+favButtons+"<a class='btn-bot' href='"+botUrl+"' target='_blank'>🤖 Comentar con @myflatscout_bot</a></div>");
                    markers.push(m);
                } catch (e) {
